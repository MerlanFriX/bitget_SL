...
class Breakout:
    """
    Breakout Indicator using Bollinger Bands and Volume Spike

    Args:
        close (pd.Series): Close price series
        volume (pd.Series): Volume series
        bb_window (int): Bollinger Bands window length
        bb_std (float): Standard deviation multiplier for bands
        volume_window (int): Rolling window for average volume
        volume_multiplier (float): Spike threshold multiplier
    """
    ...
    def _run(self):
        # Bollinger Bands
        bb = ta.volatility.BollingerBands(close=self.close, window=self.bb_window, window_dev=self.bb_std)
        self.bb_high = bb.bollinger_hband()
        self.bb_low = bb.bollinger_lband()
        self.bb_mid = bb.bollinger_mavg()

        # Volume Spike
        self.mean_vol = self.volume.rolling(self.volume_window).mean()
        self.vol_spike = self.volume > (self.mean_vol * self.volume_multiplier)

        # Breakout Conditions
        self.breakout_up = (self.close > self.bb_high) & self.vol_spike
        self.breakout_down = (self.close < self.bb_low) & self.vol_spike
    ...
    def get_bb_high(self):
        return pd.Series(self.bb_high, name="bb_high")

    def get_bb_low(self):
        return pd.Series(self.bb_low, name="bb_low")

    def get_bb_mid(self):
        return pd.Series(self.bb_mid, name="bb_mid")

    def get_vol_spike(self):
        return pd.Series(self.vol_spike, name="vol_spike")

    def get_breakout_up(self):
        return pd.Series(self.breakout_up, name="breakout_up")

    def get_breakout_down(self):
        return pd.Series(self.breakout_down, name="breakout_down")

    def get_atr(self): 
        return pd.Series(self.atr, name="atr")
