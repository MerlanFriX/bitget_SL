...
class PerpBitget:
    def __init__(self, public_api=None, secret_api=None, password=None, testnet=False):
        bitget_auth_object = {
            "apiKey": public_api,
            "secret": secret_api,
            "password": password,
        ...
        self._auth = True if secret_api else False
        self.market = None

        # Single session creation
        self._session = ccxt.bitget(bitget_auth_object) if self._auth else ccxt.bitget()
       
    async def load_markets(self):
        self.market = await self._session.load_markets()
        print("Markets loaded:", list(self.market.keys())[:5])
